...
def main():
    ...
    def handle_CompanyName():
        ...
        cur.execute("create table IF NOT EXISTS jobs_applied(company, position, date)")

        # Insert values saved into database (date as YYYY-MM-DD)
        cur.execute("insert into jobs_applied (company, position, date) values (?, ?, ?)", (name, position, date))
